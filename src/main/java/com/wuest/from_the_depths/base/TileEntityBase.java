...
import com.wuest.from_the_depths.capabilities.ITransferable;
 ...
 */
public abstract class TileEntityBase<T extends BaseConfig> extends TileEntity implements ITickable {
  protected T config;
  ...
  /**
   * Allows this class to get a class of the generic type associated with this
   * class.
   * 
  ...
  /**
   * Called from Chunk.setBlockIDWithMetadata and Chunk.fillChunk, determines if
   * this tile entity should be re-created when the ID, or Metadata changes. Use
   * with caution as this will leave straggler TileEntities, or create conflicts
  ...
  protected void saveTagData() {
    // This ensures that the tile data has been created since the write methods don't do this.
    this.getTileData();

    // Make sure to write the tile to the tag when unloading chunks.
